 ...
 */
export async function uploadAndExtract(files, prompt, format) {
 ...
 * @returns {{ answer }}
 */
export async function askQuestion(sessionId, question, files = null, format = 'json') {
  if (files && (Array.isArray(files) ? files.length > 0 : files)) {
    ...
    const { data } = await API.post('/chat_upload', formData, {
      headers: { 'Content-Type': 'multipart/form-data' },
    });
    return data;
  } else {
